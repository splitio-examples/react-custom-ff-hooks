...
function evaluateFeature(key, splitName, attributes, storage) {
  ...
  try {
    ...
    // to be evaluated
    return _promise.default.resolve(treatmentException);
  }
...
function evaluateFeatures(key, splitNames, attributes, storage) {
  ...
  try {
    ...
    return _promise.default.resolve(evaluations);
  }
  ...
  if (stringifiedSplit) {
    ...
    if ((0, _thenable.default)(evaluation)) {
      return evaluation.then(function (result) {
        ...
        result.config = (0, _lang.get)(splitJSON, "configurations.".concat(result.treatment), null);
        return result;
      });
    } else {
      evaluation.changeNumber = split.getChangeNumber(); // Always sync and optional

      evaluation.config = (0, _lang.get)(splitJSON, "configurations.".concat(evaluation.treatment), null);
    }
  }
...
function getEvaluations(splitNames, splits, key, attributes, storage) {
  ...
  var thenables = [];
  (0, _forEach.default)(splitNames).call(splitNames, function (splitName) {
    var evaluation = getEvaluation(splits.get(splitName), key, attributes, storage);
  ...
  return thenables.length > 0 ? _promise.default.all(thenables).then(function () {
    return result;
  }) : result;
}