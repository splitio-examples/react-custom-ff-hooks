...
tape('EVENTS CACHE / Should be able to add items sequentially and retrieve the queue', function (assert) {
  ...
  assert.doesNotThrow(_bindInstanceProperty(_context = cache.track).call(_context, cache, queueValues[0]), 'Calling track should not throw'); // Testing the throw on one is enough.
...
tape('EVENTS CACHE / Should not throw if the events module does not have the "flushAndResetTimer" method.', function (assert) {
  ...
  cache.track(1, 1024);
  assert.equal(cbCalled, 0, 'Cache still not full,');
  assert.doesNotThrow(_bindInstanceProperty(_context2 = cache.track).call(_context2, cache, 2), 'but when it is full, as the events module does not have the function we need, nothing happens but no exceptions are thrown.');
  assert.doesNotThrow(_bindInstanceProperty(_context3 = cache.track).call(_context3, cache, 3), 'but when it is full, as the events module does not have the function we need, nothing happens but no exceptions are thrown.');
