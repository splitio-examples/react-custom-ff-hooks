...
import mySegmentsMarcio from './mocks/mysegments.marcio@split.io.json'; // Set the mock adapter on the current axios instance
...
var settingsInMemoryWithBucketingKey = {
  core: {
    authorizationKey: '<fake-token>',
    key: {
...
tape('## E2E CI Tests ##', function (assert) {
  ...
  assert.test('E2E / In Memory', _bindInstanceProperty(evaluationsSuite).call(evaluationsSuite, null, settingsInMemory));
  assert.test('E2E / In Memory with Bucketing Key', _bindInstanceProperty(evaluationsSuite).call(evaluationsSuite, null, settingsInMemoryWithBucketingKey));
  assert.test('E2E / In LocalStorage with In Memory Fallback', _bindInstanceProperty(evaluationsSuite).call(evaluationsSuite, null, settingsInLocalStorage));
  /* Check impressions */

  assert.test('E2E / Impressions', _bindInstanceProperty(impressionsSuite).call(impressionsSuite, null, mock));
  /* Check impression listener */

  assert.test('E2E / Impression listener', impressionsListenerSuite);
  /* Check metrics */

  assert.test('E2E / Metrics', _bindInstanceProperty(metricsSuite).call(metricsSuite, null, mock));
  /* Check events */

  assert.test('E2E / Events', _bindInstanceProperty(withoutBindingTT).call(withoutBindingTT, null, mock));
  assert.test('E2E / Events with TT binded', _bindInstanceProperty(bindingTT).call(bindingTT, null, mock));
  /* Check shared clients */

  assert.test('E2E / Shared instances', _bindInstanceProperty(sharedInstantiationSuite).call(sharedInstantiationSuite, null, false, mock));
  assert.test('E2E / Shared instances with Traffic Type on factory settings', _bindInstanceProperty(sharedInstantiationSuite).call(sharedInstantiationSuite, null, true, mock));
  /* Check basic manager functionality */

  assert.test('E2E / Manager API', _bindInstanceProperty(managerSuite).call(managerSuite, null, settings, mock));
  /* Validate readiness */

  assert.test('E2E / Readiness', _bindInstanceProperty(readinessSuite).call(readinessSuite, null, mock));
  /* Validate headers for ip and hostname are not sended with requests (ignore setting IPAddressesEnabled) */

  assert.test('E2E / Ignore setting IPAddressesEnabled', _bindInstanceProperty(ignoreIpAddressesSettingSuite).call(ignoreIpAddressesSettingSuite, null, mock));
  /* Check that impressions and events are sended to backend via Beacon API or XHR when page unload is triggered. */

  assert.test('E2E / Use Beacon API (or XHR if not available) to send remaining impressions and events when browser page is unload', _bindInstanceProperty(useBeaconApiSuite).call(useBeaconApiSuite, null, mock));
  ...
  assert.test('E2E / Readiness from cache', _bindInstanceProperty(readyFromCache).call(readyFromCache, null, mock)); //If we change the mocks, we need to clear localstorage. Cleaning up after testing ensures "fresh data".
