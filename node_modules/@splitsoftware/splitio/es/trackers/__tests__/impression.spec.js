...
tape('Impression Tracker', function (t) {
  ...
  t.test('Transparently propagate the impression and attributes into a listener if provided', function (assert) {
    var fakeImpression = {
      fake: 'impression'
    };
    var fakeAttributes = {
    ...
    _setTimeout(function () {
      ...
      assert.deepEqual(fakeSettings.impressionListener.logImpression.getCall(0).args[0], _objectSpread({
        impression: fakeImpression,
        attributes: fakeAttributes,
        sdkLanguageVersion: fakeSettings.version
      }, fakeSettings.runtime), 'The listener should be executed with the corresponding map.');
      assert.end();
    }, 0);
  });
});