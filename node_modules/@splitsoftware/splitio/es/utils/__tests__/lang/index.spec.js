...
import { startsWith, endsWith, get, findIndex, find, isString, isFinite, isObject, uniqueId, merge, uniq, toString, toNumber, forOwn, groupBy, getFnName, shallowClone, isBoolean } from '../../lang';
...
tape('LANG UTILS / isObject', function (assert) {
  ...
  assert.ok(isObject(_Object$create({})), 'Should return true for map objects.');
  ...
  assert.notOk(isObject(function () {}), 'Should return false for anything that is not a map object.');
  assert.notOk(isObject(_Symbol('test')), 'Should return false for anything that is not a map object.');
  assert.notOk(isObject(new _Promise(function (res) {
    return res();
  ...
  assert.notOk(isObject(_Object$create(null)), 'Should return false for anything that is not a map object.');
  assert.end();
});
...
tape('LANG UTILS / toNumber', function (assert) {
  ...
  assert.ok(_Number$isNaN(toNumber()), 'The returned number should be NaN for values that cannot be converted');
  assert.ok(_Number$isNaN(toNumber(/regex/)), 'The returned number should be NaN for values that cannot be converted');
  assert.ok(_Number$isNaN(toNumber({})), 'The returned number should be NaN for values that cannot be converted');
  assert.ok(_Number$isNaN(toNumber({})), 'The returned number should be NaN for values that cannot be converted');
  assert.ok(_Number$isNaN(toNumber('1.2.3')), 'The returned number should be NaN for values that cannot be converted');
...
tape('LANG UTILS / isBoolean', function (assert) {
  ...
  var notBool = [null, undefined, 0, 1, NaN, Infinity, function () {}, new _Promise(function () {}), [], {}, 'true', 'false']; // negatives

  _forEachInstanceProperty(notBool).call(notBool, function (val) {
    return assert["false"](isBoolean(val));
  }); // positives


  _forEachInstanceProperty(_context = [true, false]).call(_context, function (val) {
